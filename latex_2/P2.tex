...
\begin{document}
	...
	\begin{enumerate}
		\item A random walk of $\theta$, where all parameters in the set $\theta$ are updated at once with: $\theta_proposed = \theta_old + d \quad d \sim \mathcal{N}(0,\sigma I)$
		\item A random walk of $\theta$, where only one individual parameter $theta^(i)$ is updated with: $\theta_{proposed}^(i) = \theta_old + d \quad d \sim \mathcal{N}(0,\sigma)$
	\end{enumerate}
	\begin{figure}
		\centering
